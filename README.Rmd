...
If you installed and loaded the package and whant to learn about one specific function, you can run the bare name of that function on the R console. For example, here is the source code of the function `read.csv`.

```R
read.csv
```

That may be unhelpful because often the function you care about calls a second function, and that one calls a third one, and so on. To step into such a sequence of nested functions in at least two ways. One way is to call the function on debugger mode with `debugonce()`; Debugging is beyond the scope of this article but you can read more about it [here](xxx r packages?) and [here](xxx rstudio). A second way to step into nested functions is with RStudio -- by getting the source code and jumping from function to function by pressing F2 on your keyboard.


xxx cont here

* find its repo on GitHub or other repos
